...
ds-graphd - Graphing and statistics for Fedora Directory Server
...
ds-graphd is a Perl daemon that reads access logs from Fedora
Directory Server and creates graphs of the data with RRDtool.  It
works with both the 1.0.x and 1.1.x families of Fedora DS.
...
Fedora DS access logs live in the specified directory.  The default is
C<< /var/log/dirsrv/slapd-<instance> >> (see the B<-I> flag below) on
Fedora DS 1.1.x, or C<< /opt/fedora-ds/slapd-<instance> >> on Fedora
DS 1.0.x.  The -l option should rarely be necessary; the -I option
